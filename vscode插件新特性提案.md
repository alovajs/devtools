...
基于 wormhole@2 的新特性，对 VSCode 插件进行配套升级，主要涉及状态栏交互、生成进度展示、侧边栏数据结构适配、以及自动更新配置管理的迁移。
...
当未安装 `@alova/wormhole` 时，状态栏图标当前使用灰色 `#808080`，视觉上与已禁用状态混淆。
...
将状态栏的点击命令改为新增的 `alova.statusBar.showActions` 命令，点击后弹出 QuickPick 列表，包含以下 action：

1. **Create config file** — 调用 `wormhole.createConfig({ projectPath })`
2. **Generate apis** — 调用 `wormhole.generate(config, options)`

#### 子项目选择（Monorepo 支持）

选择任意 action 后，若当前 workspace 为 monorepo 结构（`wormhole.resolveWorkspaces` 返回多个子项目），则再弹出项目选择列表：
...
利用 `wormhole.generate` 提供的 `onProgress` 回调：
...
- `ext/functions/generate.ts`：在 `wormhole.generate` 调用中传入 `onProgress`，节流后更新状态栏
...
wormhole@2 中 `getApiDocs` 的返回格式从 `Api[][]` 变更为 `CacheData[]`：
...
```ts
import type { CacheData } from '@alova/wormhole'
...
wormhole@2 去除了 `autoUpdate` 配置项和 `getAutoUpdateConfig` 导出函数，自动更新逻辑由 VSCode 扩展自行管理。

### 方案

在 VSCode 插件设置中新增 `autoUpdate` 配置项，并移除对 `wormhole.getAutoUpdateConfig` 的调用。
...
| 设置项                          | 类型      | 默认值   | 说明                 |
| ------------------------------- | --------- | -------- | -------------------- |
| `alova.autoUpdate`              | `boolean` | `true`   | 是否启用自动更新     |
| `alova.autoUpdate.interval`     | `number`  | `300000` | 自动更新间隔（ms）   |
| `alova.autoUpdate.launchEditor` | `boolean` | `false`  | 编辑器启动时立即执行 |

#### 实现

**`ext/helper/autoUpdate.ts`**：

- 移除 `wormhole.getAutoUpdateConfig(config)` 调用
- 改为读取 VSCode workspace 配置 `config.get('alova.autoUpdate')` 等字段
