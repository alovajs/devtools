...
在 `{tag}` 模板中可访问 `currentTag` 对象（类型为 [`ApiDoc`](#tagedApis)），通过 `currentTag.tag` 访问当前 tag 名称，通过 `currentTag.apis` 访问当前 tag 下的接口列表。
...
| `addNamespace`     | `(typeStr, componentNames, importName?)`  | 为类型字符串中出现的、属于 `componentNames` 的 component 名称加上命名空间前缀        |
...
在生成类型引用时，组件类型（component schema）通常通过命名空间方式导入，例如：

```typescript
import type * as ComponentTypes from "../components";
```

`addNamespace` 会扫描类型字符串，为其中出现的、且存在于 `componentNames`（[顶层字段](#顶层字段) 之一）中的 PascalCase 标识符加上命名空间前缀；不在 `componentNames` 中的标识符（如 `string`、`Record` 等）保持不变，已带前缀的标识符不会被重复添加。该 helper 对顶层类型、泛型、对象字面量和数组均一致生效。

**参数：**

- `typeStr` — 待处理的类型表达式字符串（如 `response`、`requestBody` 等字段）。
- `componentNames` — 需要加前缀的 component 名称列表，一般传入 `@root.componentNames`。
- `importName`（可选）— 引入的命名空间名称，**默认为 `ComponentTypes`**。当你的模板使用其他导入名时通过它指定。

```handlebars
{{! 默认使用 ComponentTypes 前缀 }}
type {{{name}}}Response = {{addNamespace response @root.componentNames}};
{{! 输入 Pet          → 输出 ComponentTypes.Pet }}
{{! 输入 Pet[]        → 输出 ComponentTypes.Pet[] }}
{{! 输入 List<Pet>    → 输出 List<ComponentTypes.Pet> }}
{{! 输入 string       → 输出 string（不在 componentNames 中，保持不变） }}

{{! 自定义引入名：import type * as Types from '../components'; }}
