---
title: 平台拉取器
description: 自动拼接 Swagger / Knife4j / YApi / Apifox / Postman 的 OpenAPI 文件地址
---

# 平台拉取器

worma 内置五个平台拉取器插件：`swagger`、`knife4j`、`yapi`、`apifox`、`postman`。
...
URL 型平台（`swagger` / `knife4j`）接收一个或多个基础地址，对每个地址生成候选 URL 数组：
...
1. 插件参数为平台地址（`swagger` / `knife4j`）、`yapi` 的 `{ url, pid, cookie, ... }`、`apifox` 的 `{ projectId, apifoxToken, ... }`，或 `postman` 的 `{ apiKey, collectionId }`；
