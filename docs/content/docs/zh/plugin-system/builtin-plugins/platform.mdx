---
title: 平台拉取器
description: 自动拼接 Swagger / Knife4j / FastAPI / YApi / Apifox / Postman 的 OpenAPI 文件地址
---

# 平台拉取器

worma 内置六个平台拉取器插件：`swagger`、`knife4j`、`fastapi`、`yapi`、`apifox`、`postman`。
...
| `postman(options)` | Postman 集合（需 apiKey 与 collectionId） | `{ apiKey, collectionId }` |
...
// Postman：通过 API Key 与 collection uid 拉取
...
> 各参数含义与取值可参考 Apifox 官方「参数设置」文档：[Apifox 参数设置文档](https://s.apifox.cn/apidoc/docs-site/4478210/api-173411997)
...
## Postman 插件（特殊：需 API Key）

Postman 的集合（collection）本身不是 OpenAPI 文档，需要先通过 Postman 的**集合转换接口**转换成 OpenAPI 定义：

```
https://api.getpostman.com/collections/<collectionId>/transformations
```

`postman` 插件会在 `config` hook 中把该地址写入 `config.input`，并通过 `fetchOptions` 注入 `x-api-key` 请求头；
由于该接口返回的是 `{ output: "<OpenAPI 文档>" }`，插件还会在 `beforeSpecParse` hook 中把这一层外壳解开，再交给生成器解析。

- `apiKey` 必填，作为 `x-api-key` 请求头鉴权（在 Postman → Settings → API keys 中生成）；
- `collectionId` 必填，为集合的 uid；
- 转换接口只能转换**你有权限**的集合，公开集合需先 fork 到自己的 workspace，否则会返回 403。
...
| `collectionId` | `string` | — | 集合的 uid |

## 工作原理

这些平台插件都通过 `config` hook 修改 `config.input`：

1. 插件参数为平台地址（`swagger` / `knife4j` / `fastapi`）、`yapi` 的 `{ url, pid, cookie, ... }`、`apifox` 的 `{ projectId, apifoxToken, ... }`，或 `postman` 的 `{ apiKey, collectionId }`；
2. URL 型插件根据平台类型自动拼装生成 OpenAPI 文件 URL 数组；`yapi` 根据 `url` + `pid` 拼装 `exportSwagger` 导出地址并注入 `cookie`；`apifox` 则拼装 Apifox 导出接口地址并注入 Bearer 令牌与导出参数；`postman` 则写入集合转换接口地址并注入 `x-api-key`，再在 `beforeSpecParse` 中解开返回的外壳；
