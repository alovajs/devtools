...
- 安装成功后自动清理输出目录中生成的 Skill 副本，避免同一份 Skill 被重复存储
...
type SkillAgent =
  | 'cursor' | 'claude-code' | 'codex' | 'windsurf' /* …其余助手见类型定义 */ ;
...
> worma 不再自动读取或创建任何本地配置文件（如 `node_modules/.worma/skills.local`）。安装目标完全由你通过 `agent` 字符串指定，多个助手之间自动去重。
...
> **安装成功后会清理源目录**：`skills add` 会把整个 `aidocs` 目录复制到助手自己的技能目录，为避免同一份 Skill 存两份，安装到**全部**目标助手成功后，worma 会自动删除输出目录下的 `aidocs`（如 `src/api/aidocs`）。若安装失败，或未配置 / 未解析出任何 `agent`，源目录会原样保留。
