...
If you want to keep coding agents in a separate config file (to differ per user / environment), use the exported `parseAgentFile` to parse a `key=value` file (same format as env files; lines starting with `#` are ignored). When the path is omitted, it reads `.wormaagent.local` at the project root by default:
