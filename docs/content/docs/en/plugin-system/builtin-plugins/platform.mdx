...
description: Auto-build the OpenAPI file URL for Swagger / Knife4j / FastAPI / YApi / Apifox / Postman
---

# Platform Fetchers

worma ships six platform fetcher plugins: `swagger`, `knife4j`, `fastapi`, `yapi`, `apifox`, `postman`. Their purpose is unified — pass the platform's base address (or project credentials) as a **plugin argument**, and the plugin assembles candidate OpenAPI file URLs in its `config` hook and writes them to `config.input`, so you don't have to look up and concatenate URLs manually.
...
| `fastapi(input)` | FastAPI app | Base address string, or an array of strings |
...
```typescript
import { swagger, knife4j, fastapi, yapi, apifox, postman } from 'wormajs/plugin';
...
The URL-based platforms (`swagger` / `knife4j` / `fastapi`) take one or more base addresses and generate a candidate URL array for each:
...
1. The plugin argument is the platform address (`swagger` / `knife4j` / `fastapi`), `yapi`'s `{ url, pid, cookie, ... }`, `apifox`'s `{ projectId, apifoxToken, ... }`, or `postman`'s `{ apiKey, collectionId }`;
