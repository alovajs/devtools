...
> **使用 `apifox` 插件后不再需要设置 `input` 字段**，插件会自动从 Apifox 拉取最新的 OpenAPI 文档。更多用法（按标签筛选、指定版本等）请查看 [平台拉取器](/docs/plugin-system/builtin-plugins/platform)。
...
- `outputKey`（可选）：自定义输出目录，省略时按 `src/api`、`src/api2`… 递增。
- `template`（可选）：模板类型，取值 `alova` / `axios` / `fetch` / `ky`，省略默认为 `alova`。
- `agent`（可选）：要安装 AI Skill 的编码助手，以英文逗号或中文逗号 `，` 分隔，可指定多个（自动去重）。

`.wormarc` 模式下 `aiDoc` 插件**仅在行内指定了 `agent` 时才会被添加**；未指定 `agent` 的行只生成对应模板代码，不会生成或安装 AI Skill 文档。

> 若想在 `worma.config` 中复用个人编码助手偏好，可结合 `parseAgentFile` 从 `.wormaagent.local` 读取（详见 [AI 文档生成器](/plugin-system/builtin-plugins/aiDoc)）。
