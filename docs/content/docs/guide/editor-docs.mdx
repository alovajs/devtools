...
VSCode 扩展将 wormhole 生成的 API 信息直接嵌入到编辑器中，让你无需切换工具即可完成 API 查阅和代码编写。

## 安装 VSCode 扩展

在 VSCode 扩展市场搜索 **"alova wormhole"** 并安装，或通过命令行：

```bash
code --install-extension alova.wormhole-vscode
```
...
即使项目使用纯 JavaScript，wormhole 生成的 `.d.ts` 文件仍能让 VSCode 提供完整的类型提示和文档展示：
