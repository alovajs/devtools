...
> 本文为基于《worma@2 新特性提案.md》的工程化规格说明，作为后续开发与验收依据。
> 适用范围：`packages/worma`（核心库 + CLI）、`packages/vscode-extension`（消费缓存数据）。
> 文档版本：v1.1。
>
> **实现状态图例**
>
...
- ✅ **G1 通用化**：将 worma 从 alova 专属生成器升级为通用 OpenAPI 代码生成工具。
- ✅ **G2 模板可插拔**：内置 `alova` / `alovaGlobals` / `axios` / `fetch` / `ky` 五套模板，并支持用户自定义模板。
- ✅ **G3 友好化配置**：除 `defineConfig` 外，新增 `.wormarc` 极简文本配置。
...
| 模块          | 现状路径                                                                 | 本次变更                                                                                                                                                            |
| ------------- | ------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 配置类型      | `packages/worma/src/helper/config/type.ts`（`GeneratorConfig` L130-269） | 新增/修改字段；移除 `fileNameCase`；移除 `autoUpdate`（迁移至 VSCode 扩展）                                                                                         |
| 配置 zod 校验 | `packages/worma/src/helper/config/zType.ts`                              | 同步类型变更；移除 `autoUpdate` schema                                                                                                                              |
| 配置加载      | `packages/worma/src/readConfig.ts`、`helper/config/ConfigHelper.ts`      | 新增 `.wormarc` 解析；缓存路径迁移；移除 `getAutoUpdateConfig`                                                                                                      |
| 模板预设入口  | `packages/worma/src/template/index.ts`                                   | 增加 `functional`、补全 `axios/fetch/ky`、保留 `alovaGlobals`                                                                                                       |
| 模板文件      | `packages/worma/src/template/presets/*`                                  | 新增 `alova-functional/{tag}.ts.handlebars` 等；按需引入重写 axios/fetch/ky                                                                                         |
| 模板引擎      | `packages/worma/src/helper/template/index.ts`                            | 支持 `{tag}` 文件名展开、`#` 前缀不覆盖、`partials/` 自动注册                                                                                                       |
| 解析输出      | `packages/worma/src/core/parser/templateParser/index.ts`                 | `TemplateData` 增加 `framework`/`config` 字段                                                                                                                       |
| 缓存          | `packages/worma/src/config.ts`（`cacheDir`）、`functions/wormaJson.ts`   | 路径改为项目根目录 `.worma-cache/` 目录结构；改为多 generator 结构                                                                                                  |
| VSCode 扩展   | `packages/vscode-extension/ext/views/api-server.ts` 等                   | 适配新缓存文件路径与结构；接管 `autoUpdate` 逻辑；品牌重命名为 Worma（扩展名 worma-vscode-extension，版本重置 0.0.1）                                               |
| AI 文档插件   | 新增 `packages/worma/src/plugins/presets/aiDoc/`                         | 提供 `aiDoc` 插件、Skill 模板预设（模板内容改为英文）                                                                                                               |
| CLI           | `packages/worma/src/bin/{cli,actions}.ts`                                | 参数重构：`-c/--cwd` → `-p/--project`；移除 `-w`；默认 workspace 模式；加错误处理；命令名 alova → worma                                                             |
| 品牌重命名    | 全局                                                                     | `@alova/wormhole` → `worma`；配置文件 `alova.config.*` → `worma.config.*`；`.wormarc` → `.wormarc`；缓存 `.alova-cache/` → `.worma-cache/`；全局变量 `WORMA_CONFIG` |
| 移除          | `fileNameCase`（实测代码中未实际存在该字段）                             | 在文档/类型/zod schema 中确保不会出现；保留兼容性提示                                                                                                               |
| 移除          | `autoUpdate`（`Config` 根配置项）                                        | 从 worma 核心库移除；由 VSCode 扩展自行管理自动更新逻辑                                                                                                             |
...
文件位置：项目根目录 `.wormarc`（与 `package.json` 同级）。
...
解析结果转换为等价的 `defineConfig` generator 数组，与 `worma.config.{ts,js}` 互斥（同时存在则 JS/TS 优先并打印警告）。
...
保留 `globalThis.ALOVA_WORMA_CONFIG.templateData: Map<string, TemplateData>`，键改为 generator 的 `output` 绝对路径。
...
- 位置：`packages/worma/src/template/presets/ai-doc/`。
...
- `packages/worma/src/helper/config/type.ts`：`ApiPlugin` 接口重写
- `packages/worma/src/helper/config/zType.ts`：zod schema 同步
- `packages/worma/src/helper/PluginDriver.ts`：无变化（通用调度器）
- `packages/worma/src/helper/config/GeneratorHelper.ts`：调用点适配新参数
- `packages/worma/src/helper/template/index.ts`：`generateFromTemplateDir` 返回 `files` 对象
- `packages/worma/src/plugins/presets/*`：所有预设插件适配新签名
...
`packages/worma/src/helper/config/type.ts` 新增：
...
新建 `packages/worma/src/helper/progress.ts`，导出 `ProgressTracker` 与常量 `CORE_PROGRESS_SOURCE = 'core'`、空操作 `noopReportProgress`。
...
`packages/worma/src/helper/PluginDriver.ts` 新增两个按插件构造参数的重载，保留旧 `hookSeq` / `hookParallel` 不变以避免无关回归：
...
CLI 渲染逻辑独立到 `packages/worma/src/bin/progressRenderer.ts`，导出：
...
- 新增：`packages/worma/src/helper/progress.ts`、`packages/worma/src/bin/progressRenderer.ts`
- 修改：
  - `packages/worma/src/type/lib.ts`
  - `packages/worma/src/helper/config/type.ts`
  - `packages/worma/src/helper/index.ts`
  - `packages/worma/src/helper/PluginDriver.ts`
  - `packages/worma/src/functions/prepareConfig.ts`
  - `packages/worma/src/helper/config/ConfigManager.ts`
  - `packages/worma/src/helper/config/ConfigHelper.ts`
  - `packages/worma/src/helper/config/GeneratorHelper.ts`
  - `packages/worma/src/generate.ts`
  - `packages/worma/src/bin/actions.ts`

### 8.6.9 验证 ✅

- `packages/worma/test/progress.spec.ts`：`ProgressTracker` 节流 / clamp / stop 终态 / 立刻模式（interval ≤ 0）。
- `packages/worma/test/progressRenderer.spec.ts`：bar 字符宽度、core 排序优先、消息留白、空快照。
- `packages/worma/test/pluginDriverEach.spec.ts`：`hookParallelEach` / `hookSeqEach` 注入参数。
- `packages/worma/test/generates/*` 中已有 e2e 流程加入 progress 断言（核心终态必为 100）。
...
```ts
import { swagger } from 'wormajs/plugin';
...
- 创建文件 `packages/worma/src/plugins/presets/platform.ts`
...
- `wormajs/plugin` → 现有 `rename`、`apifox`、`importType`、`filterApi`、`tagModifier`、`payloadModifier`、**新增 `aiDoc`**、**新增模板预设插件 `alova`、`alovaGlobals`、`axios`、`fetch`、`ky`**、**新增 `platform`**。
...
| `init` | `-p, --project <path>`      | `string`  | 项目目录（worma.config 所在目录），缺省为 `process.cwd()`                                         |
...
- `packages/worma/src/bin/cli.ts`：命令注册、参数定义
- `packages/worma/src/bin/actions.ts`：`actionInit`、`actionGen` 实现
- `packages/worma/test/cli.spec.ts`：CLI 单元测试
...
- **R4 `.wormarc` 与 JS/TS 配置共存策略**：当前规格选择 JS/TS 优先；需在 README 显式说明。
...
- [x] `.wormarc` 解析与 JS/TS 配置单测通过（`test/wormarc.spec.ts`）。
- [x] `.worma-cache/` 在 generate 后稳定排序、可纳入 git 且 diff 可读（`test/wormaCache.spec.ts`）。
- [x] `aiDoc` 在 functional / alovaGlobals 两种模板下产出 SKILL.md 与 reference 文档，展示 fileLocation（`test/plugins/aiDoc.spec.ts`）。
- [x] 品牌重命名全部完成：`@alova/wormhole` → `worma`；配置文件、CLI 命令、VSCode 扩展名称/命令/配置键、全局变量、函数/类型、文件名、模板、缓存目录、文档全部更新。
- [x] 单元测试全部通过（57 测试文件，613 测试用例）。
- [ ] VSCode 扩展在新缓存路径下正常渲染侧边栏 + 搜索。
- [ ] 旧用户从 worma@1 升级仅需修改 `template` 字段即可工作（提供迁移指南文档）。
- [x] CLI `worma gen` 默认 workspace 模式正确扫描子项目；`worma gen -p <path>` 单项目模式正确生成；异常时 spinner 正确终止并输出错误信息（`test/cli.spec.ts`）。
