...
被移除字段：`fileNameCase` ✅、`autoUpdate` ✅（含 `getAutoUpdateConfig` 导出函数）。
...
const plugin = aiDoc({
  ...
  install?: boolean;            // 是否调用 `skills add localpath`，默认 true
});
```
...
5. 渲染完成后调用 `skills add <outputDir>` 注册 skill（依赖 `skills` CLI；不存在则跳过并打印提示，不抛出异常）。
...
- `@alova/wormhole/plugin` → 现有 `rename`、`apifox`、`importType`、`filterApi`、`tagModifier`、`payloadModifier`、**新增 `aiDoc`**、**新增模板预设插件 `alova`、`alovaGlobals`、`axios`、`fetch`、`ky`**。
...
- **R4 与 `skills` CLI 的耦合**：`skills add` 命令不存在时静默跳过（`try/catch` 保护），不抛出异常，不影响主流程。
- **R5 `.alovarc` 与 JS/TS 配置共存策略**：当前规格选择 JS/TS 优先；需在 README 显式说明。
...
| **M5 AI Skill 插件**               | `aiDoc` 插件 + 内置 skill 模板（英文）+ skill 安装集成                               | 生成产物 + 文档校验       | ✅   |
