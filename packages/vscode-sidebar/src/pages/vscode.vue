...
// Webview 公共资源地址示例
const logoUrl = joinWebviewUri('/logo.svg')

// Vscode 主题监听和设置示例
const { theme, setTheme } = useVscTheme()
function onColortThemeInput() {
  setTimeout(() => {
...
// 网络请求示例
const whoami = ref()
async function onAxiosRequestClick() {
  // const { data } = await handlers.axiosGet('https://developer.mozilla.org/api/v1/whoami');
  whoami.value = {
    name: 'Vue Devtools',
    version: '1.0.0',
    description: 'A tool for debugging Vue applications',
  }
}
...
function onViewReactPanelOpen() {
  handlers.execCommand('alova.refresh')
}
...
<template>
  <header>
    ...
    <div class="example-block">
      <button @click="onViewReactPanelOpen()">
        更新api内容
      </button>
    </div>
    <div class="example-block">
      ...
      <input id="webview-message-input" v-model="messgeSend" type="text">
      <button @click="sendMessageToReact(messgeSend)">
        发送消息
      </button>
