...
export function recordTypeGenerator(ast: TInterface, ctx: GeneratorCtx) {
...
export function normalInterfaceGenerator(ast: TInterface, ctx: GeneratorCtx) {
  const result: GeneratorResult = {
    name: ast.keyName ?? '',
    comment: setComment(ast, ctx.options),
    type: 'interface',
...
export function interfaceTypeGenerator(ast: TInterface, ctx: GeneratorCtx) {
