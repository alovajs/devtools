...
describe('fetch template — API call execution', () => {
  ...
  describe('typescript', () => {
    it('gET with params: query params forwarded, returns data', async () => {
      ...
      mod = await buildGeneratedModule({ template: 'fetch', type: 'ts', serviceTag: 'pet' })
    ...
    it('pUT with body: sends JSON, returns response', async () => {
      ...
      mod = await buildGeneratedModule({ template: 'fetch', type: 'ts', serviceTag: 'pet' })
      const result = await mod.call('updatePet', { body: { id: 0, name: 'fetchpet', photoUrls: [] } })
      expect(receivedBody.name).toBe('fetchpet')
  ...
  describe('module', () => {
    it('gET with params: returns data', async () => {
      ...
      mod = await buildGeneratedModule({ template: 'fetch', type: 'module', serviceTag: 'pet' })
    ...
    it('pUT with body: returns response', async () => {
      let receivedBody: any = null
      getServer().use(
        http.put(`${BASE}/pet`, async ({ request }) => {
          receivedBody = await request.json()
      ...
      mod = await buildGeneratedModule({ template: 'fetch', type: 'module', serviceTag: 'pet' })
      const result = await mod.call('updatePet', { body: { id: 0, name: 'fetch-mod', photoUrls: [] } })
      expect(receivedBody.name).toBe('fetch-mod')
  ...
  describe('commonjs', () => {
    it('gET with params: returns data', async () => {
      ...
      mod = await buildGeneratedModule({ template: 'fetch', type: 'commonjs', serviceTag: 'pet' })
    ...
    it('pUT with body: returns response', async () => {
      ...
      mod = await buildGeneratedModule({ template: 'fetch', type: 'commonjs', serviceTag: 'pet' })
      const result = await mod.call('updatePet', { body: { id: 0, name: 'fetch-cjs', photoUrls: [] } })
      expect(receivedBody.name).toBe('fetch-cjs')
