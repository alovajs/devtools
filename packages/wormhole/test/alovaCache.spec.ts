...
describe('cache file (.alova-cache/)', () => {
  beforeEach(() => {
    vol.reset()
  ...
  describe('writeCacheEntry', () => {
    it('should write cache as a .alova-cache/ directory at project root', { timeout: 15000 }, async () => {
      const { writeCacheEntry } = await import('@/functions/alovaJson')
      const apis = [
      ...
      const indexContent = vol.readFileSync('/project/.alova-cache/index.json', 'utf-8') as string
    ...
    it('should write entry with path and serverName', async () => {
      ...
      const content = JSON.parse(vol.readFileSync('/project/.alova-cache/index.json', 'utf-8') as string)
    ...
    it('should merge entries when writing to an existing cache', async () => {
      ...
      vol.mkdirSync('/project/.alova-cache', { recursive: true })
      vol.writeFileSync('/project/.alova-cache/index.json', existingIndex)
      // Also need to create the data files
      vol.mkdirSync('/project/.alova-cache/data', { recursive: true })
      vol.writeFileSync('/project/.alova-cache/data/src_api.json', JSON.stringify({ serverName: 'Old', apis: [] }))
      vol.writeFileSync('/project/.alova-cache/data/src_api2.json', JSON.stringify({ serverName: 'Other', apis: [] }))
  ...
  describe('readCacheIndex', () => {
    it('should read and parse .alova-cache/index.json', async () => {
      const indexData = { schemaVersion: 1, entries: [{ path: 'src/api', serverName: 'Test', hash: 'abc', tags: {} }] }
      vol.mkdirSync('/project/.alova-cache', { recursive: true })
      vol.writeFileSync('/project/.alova-cache/index.json', JSON.stringify(indexData))
      const { readCacheIndex } = await import('@/functions/alovaJson')
  ...
  describe('readCacheApis', () => {
    it('should return null when no cache exists', async () => {
      const { readCacheApis } = await import('@/functions/alovaJson')
