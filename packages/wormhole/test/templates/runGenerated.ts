...
const WORMHOLE_ROOT = resolve(__dirname, '../..')
...
export async function buildGeneratedModule(opts: BuildOptions): Promise<GeneratedModule> {
  ...
  const tmpDir = join(tmpdir(), `wormhole-test-${template}-${type}-${serviceTag}-${Date.now()}`)
  ...
  await build({
    ...
    format: 'cjs',
    outfile: bundleOut,
    nodePaths: [join(WORMHOLE_ROOT, 'node_modules'), join(PROJECT_ROOT, 'node_modules')],
