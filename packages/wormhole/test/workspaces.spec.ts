...
describe('resolve workspaces', () => {
  it('shouldn\'t resolve any workspace when not found config in projects', async () => {
  ...
  it('should resolve a single workspace(root) when project is not monorepo', async () => {
  ...
  it('should resolve multiple workspaces when `workspaces` column in `package.json` is set', async () => {
    const rootPath = process.cwd()
  ...
  it('should resolve multiple workspaces when `pnpm-workspace.yml` is exist', async () => {
    const rootPath = process.cwd()
    const globMatches = [path.join(rootPath, 'packages/test-pkg-2'), path.join(rootPath, 'packages/test-pkg-1')]
    await createProjectStructure({
      'package.json': JSON.stringify({
...
packages:
  ...
  it('should concat the workspaces of `package.json` and `pnpm-workspace.yaml`', async () => {
    const rootPath = process.cwd()
    await createProjectStructure({
      'package.json': JSON.stringify({
        name: 'test-pkg',
