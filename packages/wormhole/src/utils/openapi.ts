 ...
 */
export const findBy$ref = <T = SchemaObject>(path: string, openApi: OpenAPIDocument, isDeep: boolean = false) => {
...
export const parseReference = <T, U = Exclude<T, ReferenceObject>>(
  obj: T,
 ...
 */
export const setComponentsBy$ref = (path: string, data: any, openApi: OpenAPIDocument) => {
  const pathArr = path.split('/');
  let find: any = {
    '#': openApi
  };
 ...
 */
export const removeAll$ref = <T = SchemaObject>(
  schemaOrigin: any,
  openApi: OpenAPIDocument,
  searchMap: Map<string, SchemaObject> = new Map()
) => {
