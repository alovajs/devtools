import { ASTType, MaybeSchemaObject, TUnknown } from '@/type';
...
import { ParserCtx, ParserOptions } from './type';
import { getParserSchemaType, parse } from './utils';

export * from './type';

export const astParse = (schema: MaybeSchemaObject, options: ParserOptions) => {
  const ctx: ParserCtx = {
    options,
    next(schema, options) {
